...
import { Box, Heading, Relative, Select, Divider } from 'rebass'
import { connect } from 'react-redux'

import { ActionButton } from './GradientButton'
import { Paragraph, Pill } from './Boxes'
...
export class QueryData extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      !clusters.length &&
      clusters.length === prevProps.clusters.length &&
  ...
  onQuery = dataset => () => {
    ...
    this.props.onQuery(selectedCluster || Object.keys(clusters)[0], dataset)
  }
  ...
  renderRow = dataset => {
    ...
    const { name, storage, slug, color } = dataset
    const isUsed = query.dataset === slug
    const onClick = isUsed ? this.onReset(dataset) : this.onQuery(dataset)
  ...
  render() {
    const { data, clusters, noTitle } = this.props
